...
module.exports = {
    ...
    async matches(req, res) {
        try{
            ...
            //const { data : { champion }} = response;
        
            //const {data} = response;

            let championsKeys = Object.keys(champions.data);
            //console.log(championsKeys);
            response.data.matches.forEach(item => {
                let champion = championsKeys.find(c => champions.data[c].key == item.champion);
                champion = champions.data[champion];
                item.championName = champion.name;
                item.imageUrl = `http://ddragon.leagueoflegends.com/cdn/10.8.1/img/champion/${champion.image.full}`;
            });
